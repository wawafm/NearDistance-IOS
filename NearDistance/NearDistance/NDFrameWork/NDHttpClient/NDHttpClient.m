               ...
               failure:(void(^)(NSError *requestErr))failure
{
    ...
    if ([[parasDict objectForKey:kHttpAllowFetchCache] boolValue]) {
        // check cache
        id cacheObj = [[NDHttpCache sharedInstance] fetchResponseForUrl:url byParam:parasDict];
        if (cacheObj) {
    ...
    if (type == NDHttpResponseType_XML) {
    ...
    else if (type == NDHttpResponseType_Json) {
    ...
    if ([[NSUserDefaults standardUserDefaults] objectForKey:kNDCookie])
    {
        [self.requestSerializer setValue:[[NSUserDefaults standardUserDefaults] objectForKey:kNDCookie] forHTTPHeaderField:@"Cookie"];
    }
    self.responseSerializer.acceptableContentTypes = [NSSet setWithObjects:@"application/json",@"text/html",@"text/plain",@"text/javascript",@"application/xml",nil];
    ...
    [self POST:requestURL parameters:transferParas success:^(AFHTTPRequestOperation *operation, id responseObject){
            ...
            else if (type == NDHttpResponseType_JqueryJson)
            {
                NSError* error = nil;
    ...
    #endif
            if (allowSaveCache == NDHttpCacheMemory || allowSaveCache == NDHttpCacheDisk) {
                [[NDHttpCache sharedInstance] storeResponse:responseObject forUrl:requestURL byParam:transferParas toDisk:allowSaveCache == NDHttpCacheMemory? NO:YES];
            }
