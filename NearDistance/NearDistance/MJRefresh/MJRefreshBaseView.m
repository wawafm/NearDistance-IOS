...
//    [self addSubview:_lastUpdateTimeLabel = [self labelWithFontSize:12]];
    ...
    // 4.箭头图片
    UIImageView *arrowImage = [[UIImageView alloc] initWithImage:[UIImage imageNamed:MJREFRESH_IMG]];
    arrowImage.frame = CGRectMake(5, 0, 27, 27);
    ...
    spinnerView.bounds = CGRectMake(0, 0, 17, 17);
    
    spinnerView.tintColor = kUIColorFromAlphaRGB(MAIN_COLOR);
    
    [self addSubview:gpRoundView = spinnerView];
...
- (UILabel *)labelWithFontSize:(CGFloat)size
{
    ...
    label.textColor = kUIColorFromRGB(GRAY_COLOR);
...
- (void)setFrame:(CGRect)frame
{
    ...
    _arrowImage.center = CGPointMake(arrowX+55, frame.size.height * 0.5+2);
    // 4.指示器
    gpRoundView.center = _arrowImage.center;
    
    _statusLabel.frame = CGRectMake(_arrowImage.right+10, statusY, statusWidth, statusHeight);
}
...
{
    CGFloat arrowX = w * 0.5 - 100;
    _arrowImage.center = CGPointMake(arrowX+55, kViewHeight * 0.5+2);
    // 4.指示器
    gpRoundView.center = _arrowImage.center;
     _statusLabel.frame = CGRectMake(_arrowImage.right+10, statusY, w, 20);
...
- (void)setState:(RefreshState)state
{
    switch (state) {
		case RefreshStateNormal:
            ...
            gpRoundView.hidden = YES;
			[gpRoundView stopAnimating];
			break;
		...
		case RefreshStateRefreshing:
            gpRoundView.hidden = NO;
			[gpRoundView startAnimating];
